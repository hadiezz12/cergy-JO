...
typedef struct {
    ...
    int position; // Assuming this is part of the Entrainement struct
    float temps;
} Entrainement;
...
void sauvegarder_entrainement(const char* nom_fichier, const Entrainement* entrainement) {
    FILE* fp = ouvrir_fichier(nom_fichier, "ab");
    fwrite(entrainement, sizeof( Entrainement), 1, fp);
    fclose(fp);
}

// Fonction pour charger les données d'entraînement depuis un fichier
void charger_entrainement(const char* nom_fichier,  Entrainement* tableau_entrainements, int* nb_entrainements) {
    FILE* fp = ouvrir_fichier(nom_fichier, "rb");
    fseek(fp, 0, SEEK_END);
    long taille = ftell(fp);
    rewind(fp);
    *nb_entrainements = taille / sizeof( Entrainement);
    fread(tableau_entrainements, sizeof( Entrainement), *nb_entrainements, fp);
    fclose(fp);
}
...
void ajouter_entrainement_athlete(const char* nom_athlete, const  Entrainement* entrainement) {
    char nom_fichier[100];
    sprintf(nom_fichier, "%s.txt", nom_athlete);
...
void afficher_historique_entrainements(const char* nom_athlete) {
     Entrainement tableau_entrainements[100]; // En supposant un maximum de 100 sessions d'entraînement
...
int entrainement_relais_existe(const char* nom_fichier, const char* date) {
    ...
    FILE* fp = ouvrir_fichier(nom_fichier, "rb");
    while (fread(&entrainement, sizeof( Entrainement), 1, fp)) {
...
//tri 
void echanger( Entrainement* a,  Entrainement* b) {
     Entrainement temp = *a; // Nous créons une variable temporaire pour stocker la valeur de a.
...
void tri_a_bulles_date( Entrainement tableau_entrainements[], int nb_entrainements) {
...
void tri_insertion_type_evenement( Entrainement tableau_entrainements[], int nb_entrainements) {
    for (int i = 1; i < nb_entrainements; ++i) { // Nous commençons par le deuxième élément du tableau.
         Entrainement cle = tableau_entrainements[i]; // Nous stockons l'élément actuel dans une variable.
...
int partition( Entrainement tableau_entrainements[], int bas, int haut) {
     Entrainement pivot = tableau_entrainements[haut]; // Nous choisissons le dernier élément comme pivot.
...
void tri_rapide_temps( Entrainement tableau_entrainements[], int bas, int haut) {
...
void calculer_progression_athlete(Athlete* athletes, int nbAthletes) {
    // Implement the logic for calculating athlete's progression here
}

void ajouter_athlete(Athlete* athletes, int* nbAthletes) {
    // Implement the logic for adding a new athlete here
}

void ajouter_session_relais(Athlete* athletes, int nbAthletes) {
    // Implement the logic for adding a relay session here
}

void consulter_historique_entrainement(Athlete* athletes, int nbAthletes) {
    // Implement the logic for displaying training history here
}
    ...
    do {
        ...
        switch (choix) {
            case 1:
                ...
                break;
            case 2:
                ajouter_session_entrainement(athletes[nbAthletes - 1].nom);  // Update as needed based on logic
                break;
            case 3:
